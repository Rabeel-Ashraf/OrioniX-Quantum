...
class TestAuths(AbstractPostgresTest):
    ...
    def setup_class(cls):
        super().setup_class()
        from OrionIX Quantum.models.auths import Auths
    ...
    def test_get_session_user(self):
        ...
        assert response.json() == {
            "id": "1",
            "name": "John Doe",
            "email": "john.doe@orionxquantumui.com",
            "role": "user",
            "profile_image_url": "/user.png",
        }

    def test_update_profile(self):
        from OrionIX Quantum.utils.auth import get_password_hash
    ...
    def test_update_password(self):
        ...
        old_auth = self.auths.authenticate_user(
            "john.doe@orionxquantumui.com", "old_password"
        )
        assert old_auth is None
        new_auth = self.auths.authenticate_user(
            "john.doe@orionxquantumui.com", "new_password"
        )
    ...
    def test_signin(self):
        ...
        response = self.fast_api_client.post(
            ...
            json={"email": "john.doe@orionxquantumui.com", "password": "password"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == user.id
        assert data["name"] == "John Doe"
        assert data["email"] == "john.doe@orionxquantumui.com"
    ...
    def test_add_user(self):
        with mock_webui_user():
            response = self.fast_api_client.post(
                ...
                json={
                    "name": "John Doe 2",
                    "email": "john.doe2@orionxquantumui.com",
                    "password": "password2",
                    "role": "admin",
                },
            )
        ...
        assert data["email"] == "john.doe2@orionxquantumui.com"
