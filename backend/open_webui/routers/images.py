    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == "openai":
            ...
            if ENABLE_FORWARD_USER_INFO_HEADERS:
                headers["X-OpenXQuantumUI-User-Name"] = quote(user.name, safe=" ")
                headers["X-OpenXQuantumUI-User-Id"] = user.id
                headers["X-OpenXQuantumUI-User-Email"] = user.email
                headers["X-OpenXQuantumUI-User-Role"] = user.role
